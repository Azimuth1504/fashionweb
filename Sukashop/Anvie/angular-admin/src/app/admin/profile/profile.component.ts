...
export class ProfileComponent implements OnInit {
  ...
  selectFile!: File;
  url: string = 'https://res.cloudinary.com/dck1sav9n/image/upload/v1748794274/default_avatar_vn5ncj.png';
  ...
  constructor(private pageService: PageService, private customerService: CustomerService, private toastr: ToastrService, private sessionService: SessionService,
    ...
    this.postForm = new FormGroup({
      ...
      'phone': new FormControl(null, [Validators.pattern('^[0-9]{10,11}$')]),
  ...
  getCustomer() {
    ...
    this.customerService.getByEmail(email).subscribe(data => {
      ...
      this.postForm = new FormGroup({
        ...
        'name': new FormControl(this.customer.name, [Validators.minLength(2), Validators.required]),
        'password': new FormControl(this.customer.password),
        'address': new FormControl(this.customer.address),
        'phone': new FormControl(this.customer.phone, [Validators.pattern('^[0-9]{10,11}$')]),
